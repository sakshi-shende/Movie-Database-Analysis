...
(SELECT rent.rental_id,
       ...
       COUNT(rent.return_date),
       rental_duration,
       amount
FROM sakila.rental rent
	JOIN sakila_snowflake.dim_customer cust
		ON rent.customer_id = cust.customer_id
    JOIN sakila_snowflake.dim_staff staff
		ON rent.staff_id = staff.staff_id
	JOIN sakila.payment payment
		ON rent.rental_id = payment.rental_id
	JOIN sakila.inventory invent
		ON rent.inventory_id = invent.inventory_id
	JOIN sakila_snowflake.dim_film d_film
		ON invent.film_id = d_film.film_id
	JOIN sakila.film film
		ON invent.film_id = film.film_id
	JOIN sakila_snowflake.dim_store store
		ON invent.store_id = store.store_id
	JOIN sakila_snowflake.dim_date d_rent
		ON DATE(rent.rental_date) = d_rent.date
	LEFT JOIN sakila_snowflake.dim_date d_return
		ON DATE(rent.return_date) = d_return.date
GROUP BY rental_id,
	customer_key,
    staff_key,
    film_key,
