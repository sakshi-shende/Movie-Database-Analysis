...
CREATE TABLE IF NOT EXISTS `sakila_snowflake`.`fact_rental` (
    ...
    `rental_duration` INT(10) NOT NULL DEFAULT 3,
	...
	CONSTRAINT `dim_customer_fact_rental_fk` FOREIGN KEY (`customer_key`)
        ...
        ON DELETE NO ACTION ON UPDATE NO ACTION,
	CONSTRAINT `dim_date_fact_rental_fk1` FOREIGN KEY (`rental_date_key`)
		REFERENCES `sakila_snowflake`.`dim_date` (`date_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
	CONSTRAINT `dim_date_fact_rental_fk2` FOREIGN KEY (`return_date_key`)
		REFERENCES `sakila_snowflake`.`dim_date` (`date_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION
);
